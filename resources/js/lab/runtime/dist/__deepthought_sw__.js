...
function getLocationPatchScript(instanceId, serverPort) {
  ...
  return `<script>
(function() {
  if (window.__deepthoughtLocPatch) return;
  ...
  try {
    ...
    if (ct.includes("text/html") && responseBody) {
      ...
      if (inspectorScript) {
        ...
        injection += `<script>window.__deepthoughtInspectConfig=${JSON.stringify({ instanceId, port: serverPort })};<` + `/script><script>${safeInspector}<` + `/script>`;
      }

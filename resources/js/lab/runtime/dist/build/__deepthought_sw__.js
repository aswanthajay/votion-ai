/**
 * DeepThoughtEngine Service Worker - proxies requests to virtual servers.
 *
 * Intercepts:
 ...
 *   /_next/*, /@vite/*, /@fs/*                guest framework assets (Next CSS
 *                                             <link> tags are often attributed
 *                                             to the host tab, not the iframe)
 ...
 * serverPort). It posts deepthought-preview-claim at document start,
...
// Real Vite origin for the Lab host (e.g. http://127.0.0.1:5173). Guest pods
// often "listen" on 5173 too — without this, host HMR/deps get proxied → 503.
const hostViteOrigins = new Set();
...
// Next (`/_next/...`) and Vite (`/@vite/...`) assets are always the guest's.
// Parser-initiated <link rel="stylesheet"> from a preview iframe often has the
// embedder (Lab host tab) as clientId; treating that as "unattributed" 404s
// CSS on the Laravel host while the HTML itself still renders.
function isGuestFrameworkAsset(pathname) {
  return (
...
function getSameOriginReferer(request) {
...
// Krikkit registers this SW at / scope for Lab preview. Host app routes and
// their assets must never route to guest pods — preview pathClaims on "/" would
// otherwise match /lab/* via ancestor walk and 503 host CSS/JS on normal reload.
function isKrikkitHostPath(pathname) {
  ...
  return (
    ...
    pathname === "/twoFactorChallenge"
  );
}

function isKrikkitHostAsset(pathname) {
  if (!pathname || pathname.charAt(0) !== "/") return false;
  return (
    ...
    pathname === "/__worker__.js" ||
    pathname.startsWith("/__worker__.js?")
  );
}

// Vite module graph on the *host* dev server (real TCP port, usually 5173).
// Must never be sent to a guest pod even when that pod also listens on 5173.
function isHostDevServerPath(pathname) {
  if (!pathname || pathname.charAt(0) !== "/") return false;
  return (
    pathname.startsWith("/@vite") ||
    pathname.startsWith("/@react-refresh") ||
...
function resetHostPreviewRoutingState() {
  pathClaims.clear();
  previewClients.clear();
  lastActivePod = null;
}

function shouldPassthroughToHost(requestUrl, refUrl) {
  if (isKrikkitHostPath(requestUrl.pathname)) return true;
  if (isKrikkitHostAsset(requestUrl.pathname)) return true;
  if (refUrl && isKrikkitHostPath(refUrl.pathname)) return true;
  return false;
}

function resolvePodForGuestAsset(clientId, refUrl) {
  if (refUrl && isKrikkitHostPath(refUrl.pathname)) {
    return null;
  }
  ...
  if (refUrl) {
    ...
    if (hit) {
      return { instanceId: hit.instanceId, serverPort: hit.port };
    }
    const claimed = lookupPodForClaimedPath(
      stripPreviewPrefix(refUrl.pathname),
    );
    if (claimed) return claimed;
  }
  return resolvePodFromLiveInstances();
}

// Next (and Vite) emit absolute http://localhost:{port}/_next/... URLs because
// the SW rewrites Host to the guest. Those never hit the Lab origin unless we
// either intercept the foreign origin here or rewrite them onto /__preview__.
function lookupPodByListenPort(port) {
  const n = Number(port);
  if (!n) return null;
  if (lastActivePod && Number(lastActivePod.serverPort) === n) {
    const set = instanceServers.get(lastActivePod.instanceId);
    if (set && set.has(n)) return lastActivePod;
  }
  const hits = [];
  for (const [instanceId, set] of instanceServers.entries()) {
    if (set && set.has(n)) hits.push({ instanceId, serverPort: n });
  }
  if (hits.length === 1) return hits[0];
  if (lastActivePod && Number(lastActivePod.serverPort) === n) return lastActivePod;
  return hits[0] || null;
}

function rewriteGuestAssetUrls(body, instanceId, serverPort) {
  const prefix = "/__preview__/" + instanceId + "/" + serverPort;
  const port = String(serverPort);
  // Next `/_next/` plus Vite absolute-from-root assets (`/src/`, `/@vite`, …).
  // Character before a guest root path in an already-prefixed URL is a digit
  // (the listen port), so ["'`(=] does not double-prefix.
  const guestRoot =
    "\\/(?:_next\\/|src\\/|node_modules\\/|@vite|@react-refresh|@id\\/|@fs\\/|__vite)";
  ...
  let out = String(body).replace(abs, prefix + "$1");
  out = out.replace(
    new RegExp("([\"'`(=])(" + guestRoot + ")", "g"),
    "$1" + prefix + "$2",
  );
  return out;
}

function shouldRewriteGuestAssets(contentType) {
  // Never rewrite JS — Next client code matches "/_next/" path prefixes.
  return /html|json|xml|x-component|text\/plain|text\/css/i.test(
    String(contentType || ""),
  );
}
 ...
 */
function looksLikeHtmlBody(bytes) {
  if (!bytes || !bytes.length) return false;
  const n = Math.min(bytes.length, 256);
  let head = "";
  try {
    head = new TextDecoder().decode(bytes.subarray(0, n));
  } catch {
    return false;
  }
  const t = head.trimStart().slice(0, 64).toLowerCase();
  return t.startsWith("<!doctype") || t.startsWith("<html") || t.startsWith("<head");
}

function inferMimeType(path, responseHeaders, body, statusCode) {
  ...
  // A Next 404 HTML page for /_next/static/css/*.css must NOT be relabeled
  // text/css — the browser would swallow it with no styles and no error.
  if (ext === ".css" && looksLikeHtmlBody(body)) return null;
...
self.addEventListener("message", (event) => {
  ...
  // Lab host shell loaded — drop stale preview pathClaims (e.g. "/") before
  // Vite bundles fetch; otherwise normal navigation to /lab/*/workspace 503s
  // app.css/lab.jsx and the Blade skeleton never gets replaced by React.
  if (data.type === "krikkit-host-active") {
    resetHostPreviewRoutingState();
    return;
  }
  if (data.type === "krikkit-host-vite" && data.origin) {
    try {
      hostViteOrigins.add(new URL(String(data.origin)).origin);
    } catch (e) {}
    return;
  }
  ...
  if (
    data.type === "deepthought-preview-claim" &&
    data.pod &&
  ...
  if (data.type === "deepthought-path-claim" && typeof data.path === "string") {
    const clientId = event.source && event.source.id;
    const pod =
...
//   2. known preview client (deepthought-preview-claim)
...
self.addEventListener("fetch", (event) => {
  ...
  // Lab host shell + assets — never proxy (sync pass-through beats stale pathClaims).
  // Vite guest paths (`/@react-refresh`, `/src/`, …) are NOT host assets; they
  // must reach isGuestFrameworkAsset / preview-client routing below.
  if (
    url.origin === self.location.origin &&
    (isKrikkitHostPath(url.pathname) || isKrikkitHostAsset(url.pathname))
  ) {
    return;
  }
  ...
  const refUrl = getSameOriginReferer(request);
  if (shouldPassthroughToHost(url, refUrl)) return;

  // 2a. Foreign loopback origin. Next may request localhost:{guestPort}/_next/...
  //     from a preview iframe. Lab's own Vite also lives on loopback (5173) and
  //     the guest often "listens" on 5173 too — never steal host Vite.
  if (url.origin !== self.location.origin) {
    if (hostViteOrigins.has(url.origin) || isHostDevServerPath(url.pathname)) {
      return;
    }
    const guestPort = Number(url.port) || (url.protocol === "https:" ? 443 : 80);
    const guestPod = lookupPodByListenPort(guestPort);
    if (guestPod) {
  ...
  // 2b. Same-origin guest framework assets (`/:8000/@react-refresh`, `/src/`…).
  //     Foreign-origin /@vite is the host Vite server (handled above).
  if (isGuestFrameworkAsset(url.pathname)) {
    const pod = resolvePodForGuestAsset(clientId, refUrl);
    if (pod) {
  ...
  if (
    clientId &&
    previewClients.has(clientId) &&
    !shouldPassthroughToHost(url, refUrl)
  ) {
  ...
  if (!isNavigation) {
    const refererPod =
      refUrl && !isKrikkitHostPath(refUrl.pathname)
...
// Routes localhost WebSocket connections through BroadcastChannel "deepthought-ws"
...
function getWsShimScript(instanceId, serverPort) {
  ...
  return `<script>
(function() {
  if (window.__krikkitLabWsShim || window.__deepthoughtWsShim) return;
  window.__krikkitLabWsShim = true;
  window.__deepthoughtWsShim = true;
  var NativeWS = window.WebSocket;
  var bc = new BroadcastChannel("deepthought-ws");
  ...
  function DeepThoughtWS(url, protocols) {
    var parsed;
  ...
  DeepThoughtWS.prototype.addEventListener = function(evt, fn) {
    if (!this._listeners[evt]) this._listeners[evt] = [];
    this._listeners[evt].push(fn);
  };
  DeepThoughtWS.prototype.removeEventListener = function(evt, fn) {
    var list = this._listeners[evt];
    if (!list) return;
    this._listeners[evt] = list.filter(function(f) { return f !== fn; });
  };
  DeepThoughtWS.prototype.dispatchEvent = function(evt) {
    _emit(this, evt.type, evt);
    return true;
  };
  DeepThoughtWS.prototype.send = function(data) {
  ...
  DeepThoughtWS.prototype.close = function(code, reason) {
  ...
  DeepThoughtWS.CONNECTING = 0;
  DeepThoughtWS.OPEN = 1;
  DeepThoughtWS.CLOSING = 2;
  DeepThoughtWS.CLOSED = 3;
  DeepThoughtWS.prototype.CONNECTING = 0;
  DeepThoughtWS.prototype.OPEN = 1;
  DeepThoughtWS.prototype.CLOSING = 2;
  DeepThoughtWS.prototype.CLOSED = 3;

  bc.onmessage = function(ev) {
    var d = ev.data;
    if (!d || !d.uid) return;
    // Filter by instance so a sibling DeepThoughtEngine's chatter doesn't leak in
  ...
  window.WebSocket = DeepThoughtWS;
})();
</script>`;
}
...
function getNavTimingPatchScript(bodyBytes) {
  ...
  return `<script>
(function() {
  if (window.__krikkitLabNavTiming || window.__deepthoughtNavTiming) return;
  window.__krikkitLabNavTiming = true;
  window.__deepthoughtNavTiming = true;
  var bodySize = ${size};
  try {
...
// posts deepthought-preview-claim at document start, controllerchange and history
...
function getLocationPatchScript(instanceId, serverPort) {
  ...
  return `<script>
(function() {
  if (window.__krikkitLabLocPatch || window.__deepthoughtLocPatch) return;
  window.__krikkitLabLocPatch = true;
  window.__deepthoughtLocPatch = true;
  ...
  var PREFIX = m ? m[0] : ('/__preview__/' + POD.instanceId + '/' + POD.serverPort);
  var guestPort = String(POD.serverPort || 0);

  // Next App Router (React 19) inserts <link rel="stylesheet"> with href
  // http://localhost:{port}/_next/static/css/... because Host was rewritten
  // to the guest. Vite does the same with /@vite, /src/, /@react-refresh.
  // Force guest CSS/JS onto the preview prefix so fetch interception can proxy them.
  function isGuestAssetPath(path) {
    return (
      path.indexOf('/_next/') === 0 ||
      path.indexOf('/src/') === 0 ||
      path.indexOf('/node_modules/') === 0 ||
      path.indexOf('/@vite') === 0 ||
      path.indexOf('/@react-refresh') === 0 ||
      path.indexOf('/@id/') === 0 ||
      path.indexOf('/@fs/') === 0 ||
      path.indexOf('/__vite') === 0
    );
  }
  function rewriteAssetUrl(u) {
    ...
    try {
      ...
      if (!isGuestAssetPath(path)) return u;
      var host = abs.hostname;
      var guest = host === 'localhost' || host === '127.0.0.1' || host === '0.0.0.0';
      var sameLab = abs.origin === location.origin;
      var portOk = !abs.port || abs.port === guestPort;
      if (sameLab || (guest && portOk)) {
        return PREFIX + path + abs.search + abs.hash;
      }
    } catch (e) {}
    if (u.charAt(0) === '/' && isGuestAssetPath(u.split('?')[0])) return PREFIX + u;
    return u;
  }
  ...
  function patchUrlProp(proto, prop) {
    ...
    while (obj && obj !== Object.prototype) {
      ...
      if (desc && typeof desc.set === 'function') break;
    ...
    Object.defineProperty(proto, prop, {
      ...
      get: desc.get,
      set: function(v) {
        desc.set.call(this, rewriteAssetUrl(String(v == null ? '' : v)));
      }
    });
  }
  try {
  ...
  Element.prototype.setAttribute = function(name, value) {
    ...
    if ((n === 'href' || n === 'src') && (tag === 'LINK' || tag === 'SCRIPT')) {
      value = rewriteAssetUrl(String(value));
    }
    return origSetAttribute.call(this, name, value);
  };

  // Next App Router defines globalThis._N_E_STYLE_LOAD so MiniCssExtractPlugin
  // skips appendChild and only records hrefs for RuntimeStyles. If we install a
  // stub function *before* app-router loads, insert() takes that path, our stub
  // resolves without recording, and CSS is lost forever (Vite unaffected — it
  // never uses _N_E_STYLE_LOAD). Only wrap after the real loader is assigned;
  // until then return undefined so MiniCssExtract falls back to appendChild
  // (our link.href / setAttribute patches still rewrite the URL).
  try {
    var _styleLoad = null;
    Object.defineProperty(globalThis, '_N_E_STYLE_LOAD', {
      configurable: true,
      enumerable: true,
      get: function() {
        if (typeof _styleLoad !== 'function') return undefined;
        return function(href) {
          href = rewriteAssetUrl(String(href == null ? '' : href));
          return _styleLoad(href);
        };
      },
      set: function(fn) { _styleLoad = fn; }
    });
  } catch (e) {}

  function fixAssetNode(n) {
    ...
    if (tag === 'LINK' || tag === 'SCRIPT') {
      ...
      if (cur) {
        var next = rewriteAssetUrl(cur);
        if (next !== cur) origSetAttribute.call(n, attr, next);
      }
    }
  }
  try {
    var mo = new MutationObserver(function(muts) {
      for (var i = 0; i < muts.length; i++) {
        var m = muts[i];
        if (m.type === 'attributes') { fixAssetNode(m.target); continue; }
        var nodes = m.addedNodes;
        for (var j = 0; j < nodes.length; j++) {
          var node = nodes[j];
          fixAssetNode(node);
          if (node.querySelectorAll) {
            var kids = node.querySelectorAll('link[href],script[src]');
            for (var k = 0; k < kids.length; k++) fixAssetNode(kids[k]);
          }
        }
      }
    });
  ...
  // Parser/React <link> can still miss the SW (foreign origin, COEP). Fetch
  // CSS through the preview prefix and inject <style> — same end state as Vite.
  function adoptStylesheet(href) {
    href = rewriteAssetUrl(String(href || ''));
    if (!href || href.indexOf('/_next/') < 0) return;
    if (document.querySelector('style[data-krikkit-css="' + href.replace(/"/g, '') + '"]')) return;
    fetch(href, { credentials: 'same-origin' }).then(function(r) {
      if (!r.ok) return null;
      return r.text();
    }).then(function(css) {
      if (!css || /<html[\\s>]/i.test(css.slice(0, 256))) return;
      var s = document.createElement('style');
      s.setAttribute('data-krikkit-css', href);
      s.textContent = css;
  ...
  function recoverCss() {
    ...
    for (var i = 0; i < links.length; i++) {
      adoptStylesheet(links[i].href || links[i].getAttribute('href'));
    }
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', recoverCss);
  } else {
    recoverCss();
  }
  setTimeout(recoverCss, 800);
  setTimeout(recoverCss, 2500);
  ...
  function claimPath() {
    try {
      var msg = { type: 'deepthought-preview-claim', pod: POD, path: strip(location.pathname) };
  ...
  try {
    ...
    console.warn('[deepthought] initial URL strip failed:', e);
  }
  claimPath();
...
// GitHub watermark badge — off in Lab (preview + workspace guest).
...
function sanitizeProxyHeaders(h, instanceId, serverPort) {
  ...
  // Guest CSS/JS requested as http://localhost:{port}/... is cross-origin
  // from the Lab iframe. A SW-synthesized response without ACAO is dropped
  // silently for destination:style (no console error, just unstyled HTML).
...
<div class="container">
  ...
  <div class="hint">Powered by DeepThoughtEngine</div>
</div>
</body>
</html>`;
...
// Boot-race retry: the preview iframe often fires its first requests before
// the dev server has registered its port with the tab. ~6s of quiet retries
// absorbs that window; navigations that still miss get a self-refreshing
// page instead of a raw "No server on ..." body.
const NO_SERVER_RETRIES = 12;
const NO_SERVER_RETRY_DELAY_MS = 500;

function isTransientNoServer(data) {
  if (!data || data.statusCode !== 503 || !data.bodyBase64) return false;
  try {
    return atob(data.bodyBase64).startsWith("No server on");
  } catch {
    return false;
  }
}
...
function bootingPage() {
  const html = `<!DOCTYPE html>
<html lang="en" data-krikkit-preview-boot="1">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
...
<body data-krikkit-preview-boot="1"></body>
</html>`;
  return new Response(html, {
    status: 503,
...
async function proxyToVirtualServer(request, instanceId, serverPort, path, originalRequest) {
  ...
  if (!targetPort) {
    // no tabs connected, poke clients to reinit and poll for the port —
    // workspace boot can take a few seconds after a hard reload.
    const clients = await self.clients.matchAll();
    for (const client of clients) {
      client.postMessage({ type: "sw-needs-init" });
    }
    for (let attempt = 0; attempt < 10 && !targetPort; attempt++) {
    ...
    if (!targetPort) {
      ...
      return errorPage(503, "Service Unavailable", "The DeepThoughtEngine service worker is still initializing. Please refresh the page.");
    }
  }
  ...
  const dispatchOnce = (tabPort) => {
    const id = nextId++;
    const promise = new Promise((resolve, reject) => {
      pending.set(id, { resolve, reject, port: tabPort });
      setTimeout(() => {
        if (pending.has(id)) {
  ...
  try {
    ...
    for (let attempt = 0; ; attempt++) {
      const promise = dispatchOnce(targetPort);
      if (!promise) {
        return errorPage(503, "Service Unavailable", "The owning tab for this server is no longer connected.");
      }
      data = await promise;
      if (!isTransientNoServer(data) || attempt >= NO_SERVER_RETRIES) break;
      await new Promise((r) => setTimeout(r, NO_SERVER_RETRY_DELAY_MS));
      // the tab may have re-attached under a fresh port while we waited
      targetPort = getPortForInstance(instanceId) || targetPort;
    }
    // Still booting after all retries: hand navigations a self-refreshing
    // page — never the raw "No server on ..." text.
    if (isTransientNoServer(data) && String(headers["accept"] || "").includes("text/html")) {
      return bootingPage();
    }
    ...
    const overrideMime = inferMimeType(
      path,
      respHeaders,
      responseBody,
      data.statusCode,
    ...
    // browser-side WebSocket connections are routed through deepthought, and
    ...
    if (ct.includes("text/html") && responseBody) {
      ...
      if (inspectorScript) {
        ...
        injection += `<script>window.__krikkitLabInspectConfig=${JSON.stringify({ instanceId, port: serverPort })};window.__deepthoughtInspectConfig=${JSON.stringify({ instanceId, port: serverPort })};<` + `/script><script>${safeInspector}<` + `/script>`;
      }
      ...
      const html = rewriteGuestAssetUrls(
    ...
    } else if (shouldRewriteGuestAssets(ct) && responseBody) {
      // RSC (text/x-component), JS chunks, and JSON also embed absolute
      // localhost:{port}/_next/... stylesheet hrefs that React hydrates later.
      try {
        const rewritten = rewriteGuestAssetUrls(
          new TextDecoder().decode(responseBody),
          instanceId,
          serverPort,
        );
        finalBody = new TextEncoder().encode(rewritten);
        for (const k of Object.keys(respHeaders)) {
          if (k.toLowerCase() === "content-length") {
            respHeaders[k] = String(finalBody.byteLength);
          }
        ...
        /* leave binary / undecodable bodies untouched */
      }
    }

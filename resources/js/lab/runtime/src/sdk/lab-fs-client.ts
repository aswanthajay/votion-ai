...
// runtime.sharedFSBuffer, postMessage it to the worker, then on the worker
// side do LabRuntime.attachFS(buffer) and you get this. reads hit the shared
...
export class LabFSClientError extends Error {
  readonly code: string;
  constructor(code: string, message: string) {
    super(message);
    this.code = code;
    this.name = "LabFSClientError";
  }
}

function notSupported(op: string): never {
  throw new LabFSClientError(
    "ENOTSUP",
    `LabFSClient is read-only, ${op}() not supported from a sibling worker. ` +
      `route the call back to the thread that booted LabRuntime and use runtime.fs.${op}() there.`,
  );
}

export class LabFSClient {
  ...
  async readFile(path: string, encoding?: string): Promise<string | Uint8Array> {
    ...
    if (bytes === null) {
      throw new LabFSClientError("ENOENT", `ENOENT: no such file, open '${path}'`);
    }
  ...
  async stat(path: string): Promise<SharedVFSStat> {
    ...
    if (s === null) {
      throw new LabFSClientError("ENOENT", `ENOENT: no such file or directory, stat '${path}'`);
    }
    return s;
  }
  ...
  async readdir(path: string): Promise<string[]> {
    ...
    if (!this._reader.existsSync(path) && path !== "/") {
      throw new LabFSClientError("ENOENT", `ENOENT: no such file or directory, scandir '${path}'`);
    }

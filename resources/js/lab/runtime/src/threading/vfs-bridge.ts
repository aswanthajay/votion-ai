// Krikkit Lab — syncs canonical MemoryVolume with worker VFS clones
...
const LAB_VFS_CHUNK_BYTES = 4 * 1024 * 1024; // 4MB
...
export class VFSBridge {
  ...
  createChunkedSnapshots(): { chunkIndex: number; totalChunks: number; data: ArrayBuffer; manifest: VFSSnapshotEntry[] }[] {
    ...
    if (totalSize <= LAB_VFS_CHUNK_BYTES) {
      return [{
        chunkIndex: 0,
        totalChunks: 1,
    ...
    for (const entry of fullSnapshot.manifest) {
      ...
      if (chunkBytes > 0 && chunkBytes + entry.length > LAB_VFS_CHUNK_BYTES) flush();

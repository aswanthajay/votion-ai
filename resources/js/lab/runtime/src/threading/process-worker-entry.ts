// per-process Web Worker entry — each worker gets its own MemoryVolume, ScriptEngine, and DeepThoughtShell
...
function workerThreadFork(
  ...
  _ipcCallbacks.set(requestId, (data: unknown) => {
    ...
    const workerError = readWorkerError(data);
    if (workerError !== null) {
      opts.onError(new Error(workerError));
      return;
    }
    opts.onMessage(data);
  });

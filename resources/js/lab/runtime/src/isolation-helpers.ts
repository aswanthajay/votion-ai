...
function readServiceWorkerSource(): string | null {
  try {
    ...
    let swFile = path.join(dir, '__krikkit_lab_sw__.js');
    if (fs.existsSync(swFile)) return fs.readFileSync(swFile, 'utf-8');
    swFile = path.join(dir, '../dist/__krikkit_lab_sw__.js');
    if (fs.existsSync(swFile)) return fs.readFileSync(swFile, 'utf-8');
    swFile = path.join(dir, '__deepthought_sw__.js');
    if (fs.existsSync(swFile)) return fs.readFileSync(swFile, 'utf-8');
    swFile = path.join(dir, '../dist/__deepthought_sw__.js');
    if (fs.existsSync(swFile)) return fs.readFileSync(swFile, 'utf-8');
    return null;
  } catch {
    return null;
...
export interface SandboxPageConfig {
  /** Absolute URL to the Lab runtime bundle on your origin (required). */
  runtimeUrl: string;
  /** @deprecated Use runtimeUrl */
  deepThoughtUrl?: string;
...
export function getSandboxPageHtml(config: SandboxPageConfig | string): string {
  const opts: SandboxPageConfig =
    typeof config === "string" ? { runtimeUrl: config } : config;
  const runtimeUrl = (opts.runtimeUrl ?? opts.deepThoughtUrl)?.trim();
  if (!runtimeUrl) {
    throw new Error(
      "[LabRuntime] runtimeUrl is required — host the runtime bundle on your origin (no npm CDN fallback).",
    );
  }
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('/__krikkit_lab_sw__.js', { scope: '/' })
...
` : '';
  ...
  return `<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><title>Krikkit Lab Sandbox</title></head>
<body>
<script type="module">
  import { MemoryVolume, ScriptEngine } from '${runtimeUrl}';
${swBlock}
...
export function generateSandboxDeployment(config: SandboxPageConfig | string): GeneratedSandboxFiles {
  ...
  if (swSource) files['__deepthought_sw__.js'] = swSource;
  return files;
}

export const SANDBOX_DEPLOYMENT_GUIDE = `
# Deploying a Krikkit Lab Sandbox

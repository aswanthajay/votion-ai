...
import deepthought from "../../integrations/vite";
...
describe("integrations/vite", () => {
  it("factory returns a Vite plugin object with expected hooks", () => {
    const plugin = deepthought();
    expect(plugin.name).toBe("deepthought");
    expect(typeof plugin.configureServer).toBe("function");
    expect(typeof plugin.generateBundle).toBe("function");
  });

  it("respects a custom path option", () => {
    const plugin = deepthought({ path: "/custom-sw.js" });
    expect(plugin.name).toBe("deepthought");
  });

  it("emits __deepthought_sw__.js as an asset during generateBundle", async () => {
    const plugin = deepthought();
    ...
    const sw = emitted.find((a) => a.fileName === "__deepthought_sw__.js");
  ...
  it("configureServer mounts a middleware that serves the SW at /__deepthought_sw__.js", async () => {
    const plugin = deepthought();
    ...
    // /__deepthought_sw__.js: SW source written with JS Content-Type + scope headers.
    {
      const res = new MockRes();
      await mw({ url: "/__deepthought_sw__.js" }, res, () => {
    ...
    {
      const res = new MockRes();
      await mw({ url: "/__deepthought_sw__.js?v=12345" }, res, () => {
...
// End-to-end: spin up a real Vite dev server, hit /__deepthought_sw__.js over HTTP.
describe("integrations/vite end-to-end", () => {
  it("dev server serves /__deepthought_sw__.js with real HTTP fetch", async () => {
    ...
    try {
      ...
      const url = `http://127.0.0.1:${addr.port}/__deepthought_sw__.js`;
  ...
  it("production build emits __deepthought_sw__.js as a rollup asset", async () => {
    // Vite lib mode resolves the entry before plugins run, so a virtual
    // module won't work. Point it at a real file in a temp dir.
    const dir = await mkdtemp(join(tmpdir(), "deepthought-vite-test-"));
    ...
    try {
      ...
      const sw = assets.find(
        (a) => a.type === "asset" && a.fileName === "__deepthought_sw__.js",
      );
      expect(sw).toBeDefined();
      if (sw && sw.type === "asset") {

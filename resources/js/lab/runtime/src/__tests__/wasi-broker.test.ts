...
describe("tab-realm WASI worker broker", () => {
  ...
  it("recovers package-relative /lightningcss-wasm opens via node_modules CDN mapping", async () => {
    const volume = new MemoryVolume();
    volume.mkdirSync("/node_modules/lightningcss-wasm", { recursive: true });
    volume.writeFileSync("/node_modules/lightningcss-wasm/package.json", JSON.stringify({
      name: "lightningcss-wasm",
      version: "1.30.2",
    }));
    volume.writeFileSync(
      "/node_modules/lightningcss-wasm/lightningcss_node.js",
      "module.exports = {}",
    );
    const wasm = new Uint8Array([0, 97, 115, 109, 1, 0, 0, 0]);
    vi.stubGlobal("fetch", vi.fn(async () => new Response(wasm, {
    ...
    (manager as any)._handleFsProxyWithRecovery(
      { sab: control, type: "existsSync", payload: ["/lightningcss-wasm/lightningcss_node.wasm"], requestId: 1 },
      buildFileSystemBridge(volume, () => "/"),
    );

    await vi.waitFor(() => expect(Atomics.load(control, 0)).toBe(0));
    expect(volume.existsSync("/node_modules/lightningcss-wasm/lightningcss_node.wasm")).toBe(true);
    expect(fetch).toHaveBeenCalledWith(
      "https://cdn.jsdelivr.net/npm/lightningcss-wasm@1.30.2/lightningcss_node.wasm",
    );
  });
  ...
  it("queues early messages, rejects duplicates, and cleans up exactly once", () => {
    ...
    worker.emitMessage({ __deepthought_broker_ready__: true });
    expect(worker.messages).toContain("early");
    expect(handle.sent).toContainEqual({
      type: "ipc-message",
      targetRequestId: 7,
      data: { __deepthought_broker_ready__: true },
    });
  ...
  it("runs more than sixteen sequential workers without retaining broker state", () => {
    ...
    for (let requestId = 1; requestId <= 20; requestId++) {
      ...
      FakeWorker.instances.at(-1)!.emitMessage({ __deepthought_broker_ready__: true });
      FakeWorker.instances.at(-1)!.emitMessage({ __deepthought_worker_exit__: 0 });
      expect(manager.resourceStats().workers).toBe(0);
    }
  ...
  it("times out a worker that never completes its ready handshake", () => {
    ...
    expect(handle.sent).toContainEqual(expect.objectContaining({
      type: "ipc-message",
      targetRequestId: 9,
      data: { __deepthought_worker_error__: "WASI worker ready handshake timed out" },
    }));
  });
  ...
  it("cleans up immediately when worker initialization reports an error", () => {
    ...
    const worker = FakeWorker.instances[0];
    worker.emitMessage({
      __deepthought_worker_error__: "Error: missing wasm asset",
    });
  ...
  it("spawns delegated emnapi threads without waiting on the process worker", () => {
    ...
    root.emitMessage({ __deepthought_broker_ready__: true });
    ...
    child.emitMessage({ __deepthought_broker_ready__: true });

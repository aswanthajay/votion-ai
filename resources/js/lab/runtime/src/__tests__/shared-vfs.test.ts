...
import { DeepThoughtFSClient, DeepThoughtFSClientError } from "../sdk/deepthought-fs-client";
...
describe("DeepThoughtFSClient", () => {
  it("readFile returns bytes", async () => {
    const ctrl = new SharedVFSController();
    ctrl.writeFile("/data.bin", new Uint8Array([9, 8, 7]));

    const client = new DeepThoughtFSClient(new SharedVFSReader(ctrl.buffer));
  ...
  describe("write operations throw ENOTSUP", () => {
    ...
    it("throws DeepThoughtFSClientError (named)", async () => {
      try {
        await client.writeFile("/x", "y");
        throw new Error("should have thrown");
      } catch (e) {
        expect(e).toBeInstanceOf(DeepThoughtFSClientError);
      }
    });
  });

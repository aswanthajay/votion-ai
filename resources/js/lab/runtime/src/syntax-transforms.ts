// Krikkit Lab — ESM-to-CJS transforms (acorn AST + regex fallback)

import * as acorn from "acorn";

// Pre-compiled regex patterns for fallback paths (avoid per-call compilation)
const SX_AWAIT_QUICK = /\bawait\b/;
const SX_ASYNC_QUICK = /\basync\b/;
const SX_AWAIT_LOOKAHEAD = /(?<![.\w])await\s+(?=[\w$("'\[`!~+\-/])/;
const SX_AWAIT_LOOKAHEAD_G = /(?<![.\w])await\s+(?=[\w$("'\[`!~+\-/])/g;
const SX_ASYNC_FN_G = /(?<![.\w])async\s+(?=function[\s*(])/g;
const SX_ASYNC_PAREN_G = /(?<![.\w])async\s+(?=\()/g;
const SX_ASYNC_ARROW_G = /(?<![.\w])async\s+(?=\w+\s*=>)/g;
const SX_TYPE_IMPORT_BRACES = /import\s+type\s+\{[^}]*\}\s+from\s+['"][^'"]+['"]\s*;?/g;
const SX_TYPE_IMPORT_DEFAULT = /import\s+type\s+\w+\s+from\s+['"][^'"]+['"]\s*;?/g;
const SX_TYPE_IMPORT_STAR = /import\s+type\s+\*\s+as\s+\w+\s+from\s+['"][^'"]+['"]\s*;?/g;
const SX_MIXED_TYPE_IMPORT = /import\s+\{([^}]*\btype\s+\w+[^}]*)\}\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_TYPE_EXPORT_FROM = /export\s+type\s+\{[^}]*\}\s+from\s+['"][^'"]+['"]\s*;?/g;
const SX_TYPE_EXPORT = /export\s+type\s+\{[^}]*\}\s*;?/g;
const SX_IMPORT_STAR = /import\s+\*\s+as\s+(\w+)\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_IMPORT_DEFAULT_NAMED = /import\s+(\w+)\s*,\s*\{([^}]+)\}\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_IMPORT_DEFAULT = /import\s+(\w+)\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_IMPORT_NAMED = /import\s+\{([^}]+)\}\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_IMPORT_SIDE_EFFECT = /import\s+['"]([^'"]+)['"]\s*;?/g;
const SX_EXPORT_DEFAULT_CLASS = /export\s+default\s+class\s+(\w+)/g;
const SX_EXPORT_DEFAULT_FN_NAMED = /export\s+default\s+function\s+(\w+)/g;
const SX_EXPORT_DEFAULT_FN_ANON = /export\s+default\s+function\s*\(/g;
const SX_EXPORT_DEFAULT = /export\s+default\s+/g;
const SX_EXPORT_STAR_AS = /export\s+\*\s+as\s+(\w+)\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_EXPORT_STAR = /export\s+\*\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_EXPORT_NAMED_FROM = /export\s+\{([^}]+)\}\s+from\s+['"]([^'"]+)['"]\s*;?/g;
const SX_EXPORT_NAMED = /export\s+\{([^}]+)\}\s*;?/g;
const SX_EXPORT_ASYNC_FN = /export\s+async\s+function\s+(\w+)/g;
const SX_EXPORT_FN = /export\s+function\s+(\w+)/g;
const SX_EXPORT_CLASS = /export\s+class\s+(\w+)/g;
const SX_EXPORT_VAR = /export\s+(?:const|let|var)\s+(\w+)\s*=/g;
const SX_AS_RENAME = /(\w+)\s+as\s+(\w+)/g;
const SX_TYPE_SPEC = /^\s*type\s+\w+/;
const SX_AS_SPLIT = /\s+as\s+/;
...
export function hasTopLevelAwait(code: string): boolean {
  if (!SX_AWAIT_QUICK.test(code)) return false;
  ...
  try {
    ...
    return SX_AWAIT_LOOKAHEAD.test(code);
  }
}
...
export function stripTopLevelAwait(
  ...
  if (!full && !SX_AWAIT_QUICK.test(code)) return code;
  if (full && !SX_AWAIT_QUICK.test(code) && !SX_ASYNC_QUICK.test(code)) return code;
  ...
  try {
    ...
    if (full) {
      let out = code.replace(SX_AWAIT_LOOKAHEAD_G, "");
      out = out.replace(SX_ASYNC_FN_G, "");
      out = out.replace(SX_ASYNC_PAREN_G, "");
      out = out.replace(SX_ASYNC_ARROW_G, "");
      return out;
    }
    return code.replace(SX_AWAIT_LOOKAHEAD_G, "");
  }
}
...
function esmToCjsViaRegex(
  ...
  out = out.replace(SX_TYPE_IMPORT_BRACES, "");
  out = out.replace(SX_TYPE_IMPORT_DEFAULT, "");
  out = out.replace(SX_TYPE_IMPORT_STAR, "");
  // remove inline type specifiers from mixed imports
  out = out.replace(
    SX_MIXED_TYPE_IMPORT,
    (_m, specs: string, src: string) => {
      const kept = specs
        .split(",")
        .filter((s: string) => !SX_TYPE_SPEC.test(s))
        .map((s: string) => s.trim())
        .filter(Boolean);
      if (kept.length === 0) return "";
      const fixed = kept.join(", ").replace(SX_AS_RENAME, "$1: $2");
      return `const {${fixed}} = require("${src}");`;
    },
  );
  // strip TS type-only exports
  out = out.replace(SX_TYPE_EXPORT_FROM, "");
  out = out.replace(SX_TYPE_EXPORT, "");
  out = out.replace(SX_IMPORT_STAR, 'const $1 = require("$2");');
  out = out.replace(
    SX_IMPORT_DEFAULT_NAMED,
    (_m, def, named, src) => {
      const tmp = `__import_${def}`;
      const fixed = named.replace(SX_AS_RENAME, "$1: $2");
      return `const ${tmp} = require("${src}"); const ${def} = ${tmp}.__esModule ? ${tmp}.default : ${tmp}; const {${fixed}} = ${tmp};`;
    },
  );
  out = out.replace(SX_IMPORT_DEFAULT, 'const $1 = require("$2");');
  out = out.replace(
    SX_IMPORT_NAMED,
    (_m, specs, src) => {
      const fixed = specs.replace(SX_AS_RENAME, "$1: $2");
      return `const {${fixed}} = require("${src}");`;
    },
  );
  out = out.replace(SX_IMPORT_SIDE_EFFECT, 'require("$1");');
  // export default
  out = out.replace(SX_EXPORT_DEFAULT_CLASS, `${exportTarget} = class $1`);
  out = out.replace(SX_EXPORT_DEFAULT_FN_NAMED, `${exportTarget} = function $1`);
  out = out.replace(SX_EXPORT_DEFAULT_FN_ANON, `${exportTarget} = function(`);
  out = out.replace(SX_EXPORT_DEFAULT, `${exportTarget} = `);
  // re-exports
  out = out.replace(SX_EXPORT_STAR_AS, 'exports.$1 = require("$2");');
  out = out.replace(SX_EXPORT_STAR, 'Object.assign(exports, require("$1"));');
  out = out.replace(
    SX_EXPORT_NAMED_FROM,
    (_m, specs, src) => {
      const binds = specs
        .split(",")
        .map((s: string) => {
          const parts = s.trim().split(SX_AS_SPLIT);
  ...
  // named exports
  out = out.replace(SX_EXPORT_ASYNC_FN, "exports.$1 = async function $1");
  out = out.replace(SX_EXPORT_FN, "exports.$1 = function $1");
  out = out.replace(SX_EXPORT_CLASS, "exports.$1 = class $1");
  out = out.replace(SX_EXPORT_VAR, "exports.$1 =");
  return out;
}

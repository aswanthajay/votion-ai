/**
 * generic support for napi-rs WASM packages in deepthought
 *
...
import {
  LAB_THREAD_ID_TOKEN,
  LAB_WORKER_DATA_GLOBAL,
  LAB_WORKER_ERROR,
  LEGACY_THREAD_ID_TOKEN,
  LEGACY_WORKER_DATA_GLOBAL,
  LEGACY_WORKER_ERROR,
  replaceThreadIdTokens,
...
try {
  ...
  const __deepthoughtWorkerError = String(e && e.stack || e);
  console.error('[worker] Entry point failed:', __deepthoughtWorkerError);
  self.postMessage({ ${LAB_WORKER_ERROR}: __deepthoughtWorkerError, ${LEGACY_WORKER_ERROR}: __deepthoughtWorkerError });
  throw e;
}
`);
  }
      ...
      if (!sabEnabled) {
        queueMicrotask(() => {
          this.emit?.(
            "error",
            new Error(
              `[DeepThoughtEngine] ${scriptStr} needs SharedArrayBuffer (threaded wasi module). ` +
    ...
    if (!fallbackWorkerFn) {
      queueMicrotask(() => {
        this.emit?.(
          "error",
          new Error(
            "[DeepThoughtEngine] worker_threads.Worker requires worker mode for non-WASI scripts.",
          ),
        );
      });
      return;
...
function createRealWebWorker(
  ...
  const workerBundleSource = replaceThreadIdTokens(bundleSource, self.threadId);
...
function WORKER_PREAMBLE(env: Record<string, string>): string {
  ...
  return `
// === deepthought napi-rs WASI worker preamble ===
"use strict";
...
const __reportWorkerError = (err) => {
  ...
  try { __nativePostMessage({ ${LAB_WORKER_ERROR}: message, ${LEGACY_WORKER_ERROR}: message }); } catch {}
  return message;
};
...
const __DisabledWorker = class Worker {
  constructor() { throw new Error('Direct Worker creation not available in deepthought child worker (use emnapi child-thread delegation)'); }
};
...
const __builtinRequire = function(id) {
  ...
  if (bare === 'worker_threads') {
    return {
      ...
      isMainThread: false,
      workerData: globalThis.${LAB_WORKER_DATA_GLOBAL} ?? globalThis.${LEGACY_WORKER_DATA_GLOBAL} ?? null,
      threadId: ${LAB_THREAD_ID_TOKEN},
...
// The main thread handles __fs__ messages using the real deepthought VFS (MemoryVolume).
...
let __wasiStub;
${getWasiRuntimeSource("__deepthoughtSharedWasi")}
const __SharedWASI = globalThis.__deepthoughtSharedWasi.WASI;

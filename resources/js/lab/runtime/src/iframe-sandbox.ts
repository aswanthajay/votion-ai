...
export class IframeSandbox implements IScriptEngine {
  ...
  private bindMessageHandler(): void {
    this.onMessage = (event: MessageEvent) => {
      ...
      const msg = event.data as IframeSandboxMessage;
  ...
  private awaitReady(): Promise<void> {
    return new Promise(resolve => {
      const handler = (event: MessageEvent) => {
        if (event.origin !== 'null' && event.origin !== this.targetOrigin) return;
        if ((event.data as IframeSandboxMessage).type === 'ready') {
  ...
  private async sendInit(): Promise<void> {
    const msg: IframeSandboxMessage = {
      type: 'init',
  ...
  private attachVolumeSync(): void {
    this.onFileChange = (path, content) => {
      this.frame.contentWindow?.postMessage({ type: 'syncFile', path, content } as IframeSandboxMessage, '*');
    };
    this.vol.on('change', this.onFileChange);

    this.onFileDelete = (path) => {
      this.frame.contentWindow?.postMessage({ type: 'syncFile', path, content: null } as IframeSandboxMessage, '*');
    };
    this.vol.on('delete', this.onFileDelete);
  }

  private dispatch(msg: IframeSandboxMessage): Promise<ExecutionOutcome> {
  ...
  clearCache(): void {
    this.frame.contentWindow?.postMessage({ type: 'clearCache' } as IframeSandboxMessage, '*');
  }

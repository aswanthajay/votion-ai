...
// multiplexed across N DeepThoughtEngines by instanceId. each DeepThoughtEngine attach()/detach()s
...
import { DEFAULT_SW_PATH } from "./internal/lab-keys";

export { DeepThoughtSWSetupError };
export type { DeepThoughtSWFrameworkHint } from "./integrations/shared/errors";
...
export interface ServiceWorkerConfig {
  swUrl?: string;
  /**
   * Skip the HEAD preflight that checks /__deepthought_sw__.js is served as JS.
...
export class RequestProxy extends EventEmitter {
  ...
  private _watermarkEnabled = false;
  ...
  constructor(opts: ProxyOptions = {}) {
    ...
    // to DEFAULT_INSTANCE. DeepThoughtEngine SDK uses register(instanceId, ...) instead
  ...
  /** attach a DeepThoughtEngine to this proxy. idempotent: re-attaching with the same
  ...
  // HEAD /__deepthought_sw__.js so a broken setup fails loud with a useful message
  ...
  private async _preflightServiceWorker(swPath: string): Promise<void> {
    ...
    try {
      ...
      throw new DeepThoughtSWSetupError(
   ...
   *  SW-registration storm when N DeepThoughtEngines boot in parallel.
   *
  ...
  private async _doInitServiceWorker(
    ...
    const swPath = config?.swUrl ?? DEFAULT_SW_PATH;
    ...
    navigator.serviceWorker
      ...
      .catch((err) => {
        console.warn("[DeepThoughtEngine] SW register() rejected:", err);
      });
    ...
    // ready. common case: DeepThoughtEngine constructor runs before initServiceWorker
  ...
  // listens on BroadcastChannel "deepthought-ws" for connect/send/close from preview
  ...
  private _startWsBridge(): void {
    ...
    this._wsBridge = new BroadcastChannel("deepthought-ws");
  ...
  private _handleWsConnect(
    ...
    // which ignores instanceId and can route to another DeepThoughtEngine's listener.
    const entry = inst.registry.get(port);

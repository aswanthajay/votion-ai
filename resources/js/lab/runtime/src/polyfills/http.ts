  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      try {
        ...
        const line = `[deepthought] dispatchRequest TIMEOUT after ${timeoutMs}ms for ${verb} ${target}, middleware never responded\n`;
    ...
    const reportErr = (kind: "async" | "sync", err: unknown) => {
      ...
      const line = `[deepthought] dispatchRequest ${kind} error on ${verb} ${target}: ${msg}\n${stack}\n`;

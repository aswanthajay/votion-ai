...
export interface BundleConfig {
  entryPoints?: Array<string | { in: string; out?: string }>;
  stdin?: {
    contents: string;
    resolveDir?: string;
    loader?: "js" | "jsx" | "ts" | "tsx" | "json" | "css";
...
export async function build(cfg: BundleConfig): Promise<BundleOutput> {
  ...
  try {
    ...
    if (entries && volumeRef) {
      ...
      entries = entries.map((ep) => normalizeEntryPoint(ep, base));
    }
...
// Browser can't block on WASM transform. For TS/TSX loaders, strip types
// synchronously so require()/config loaders never eval raw TypeScript.
export function transformSync(
  ...
  if (loader === "ts" || loader === "tsx") {
    const filename = loader === "tsx" ? "module.tsx" : "module.ts";
    return {
      code: stripTypeScript(source, filename),
      map: "",
      warnings: [],
    };
  }
...
// Node/worker resolution must NOT prefer "browser" over "default" — packages like
// decode-named-character-reference ship index.dom.js under "browser" which needs
// document.createElement and blows up inside the process worker.
...
/** esbuild allows `{ in, out }` entry objects — Next compileConfig uses them. */
function normalizeEntryPoint(
  entry: string | { in?: string; out?: string; [key: string]: unknown },
  base: string,
): string | { in?: string; out?: string; [key: string]: unknown } {
  if (typeof entry === "string") return toAbsolute(entry, base);
  if (entry && typeof entry === "object") {
    const next = { ...entry };
    if (typeof next.in === "string") next.in = toAbsolute(next.in, base);
    return next;
  }
  return entry;
}

...
const locateBundledWaSqlite = () => "https://deepthought.invalid/wa-sqlite.wasm";
...
const DEEPTHOUGHT_VFS_NAME = "deepthought";
...
function tempDbPath(prefix: string): string {
  return `/.__deepthought_${prefix}_${Date.now()}_${Math.random().toString(36).slice(2)}.db`;
}
...
class DeepThoughtEngineVFS {
  name = DEEPTHOUGHT_VFS_NAME;
  mxPathName = 512;
...
export const WASM_CACHE_PATH = "/.deepthought/wa-sqlite.wasm";
...
let deepThoughtVfs: DeepThoughtEngineVFS | null = null;
...
function buildEngineFromWaModule(waModule: WaModule): SqliteEngine {
  ...
  deepThoughtVfs = new DeepThoughtEngineVFS();
  api.vfs_register(deepThoughtVfs, false);
...
export class DatabaseSync implements DatabaseSyncInterface {
  ...
  open(): void {
    ...
    const { sync } = getEngine();
    const vfs =
      this.resolvedPath === ":memory:" ? undefined : DEEPTHOUGHT_VFS_NAME;
...
function copyDatabaseToFile(db: DatabaseSync, destPath: string): void {
  ...
  const dest = sync.open(destPath, DEEPTHOUGHT_VFS_NAME);
  try {
    const objects = db
      .prepare("SELECT sql FROM sqlite_master WHERE sql NOT NULL ORDER BY rowid")
      .all() as Array<{ sql: string }>;

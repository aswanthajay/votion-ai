...
function patchTimerContext(): void {
  ...
  if (!(globalThis.queueMicrotask as any)?.__deepthoughtAsyncCtx) {
    const orig = globalThis.queueMicrotask.bind(globalThis);
    globalThis.queueMicrotask = ((cb: () => void) => {
      orig(wrapTimer(cb));
    }) as typeof queueMicrotask;
    (globalThis.queueMicrotask as any).__deepthoughtAsyncCtx = true;
  }

  if (!(globalThis.setTimeout as any)?.__deepthoughtAsyncCtx) {
    ...
    (globalThis.setTimeout as any).__deepthoughtAsyncCtx = true;
  }

  if (typeof globalThis.setImmediate === "function" && !(globalThis.setImmediate as any).__deepthoughtAsyncCtx) {
    ...
    (globalThis.setImmediate as any).__deepthoughtAsyncCtx = true;
  }
}
...
export function installAsyncContext(): void {
  ...
  if (!(globalThis.Promise as any)?.__deepthoughtAsyncCtx) {
    (globalThis as any).Promise = ContextPromise;
    (ContextPromise as any).__deepthoughtAsyncCtx = true;
  }
...
(AsyncLocalStorage as any).__deepthoughtAsyncCtx = true;
...
const EXCLUDED: unique symbol = Symbol("deepthought.als.excluded");

// deepthought - browser-native Node.js runtime environment

// Default RuntimeHost for the browser entry. The browser-host module also
// publishes `globalThis.__DEEPTHOUGHT_CREATE_BROWSER_HOST__` so boot still works
...
import { publishGlobalBrowserHostFactory } from "./internal/lab-keys";
...
export function createWorkspace(config?: WorkspaceConfig): {
  ...
  // init shell lazily (SDK path uses DeepThoughtEngine.boot() instead)

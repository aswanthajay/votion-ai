/** Krikkit Lab boot/runtime performance counters. */
...
function quantizeMs(value: number): number {
...
export class PerformanceTracker {
  ...
  snapshot(): PerformanceStats {
    ...
    for (const [name, timing] of this.timings) {
      timings[name] = {
        count: timing.count,
        totalMs: quantizeMs(timing.totalMs),
        lastMs: quantizeMs(timing.lastMs),
        minMs: quantizeMs(timing.minMs),
        maxMs: quantizeMs(timing.maxMs),
      };
    }
    return {
      timings,
